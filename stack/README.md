...
| Type checker | **tsgo** (`@typescript/native-preview`) | Track the bleeding edge — TS 6/7 betas. `typecheck` script is `tsgo`. |
...
Keep script names consistent across repos: `start`, `dev`, `typecheck` (`tsgo`),
...
- **Bun library / CLI monorepo** — the dominant style. Everything above applies. Start from `config/package.template.json`, add `packages/<pkg>` with the dual-condition exports.
- **npm dual-publish lib** — `tsup` (`format: ['cjs','esm']`), Volta pins Node, `.npmrc` with `fund=false`/`audit=false`. Older style; prefer the Bun monorepo for new work.
